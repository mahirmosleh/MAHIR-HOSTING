...
app.secret_key = 'MAHIR-jod-super-secret-key-2026'
...
def load_users():
    if not os.path.exists(USERS_FILE):
        default = {"MAHIR": {"password": "MAHIRJOD", "role": "admin", "servers": []}}
        save_users(default)
        return default
    with open(USERS_FILE, 'r', encoding='utf-8') as f:
        data = json.load(f)
    # সব ইউজারের জন্য 'servers' নিশ্চিত করুন
    for uname, udata in data.items():
        if 'servers' not in udata:
            udata['servers'] = []
    # অ্যাডমিন চেক
    if 'admin' not in data:
        data['MAHIR'] = {"password": "MAHIRJOD", "role": "admin", "servers": []}
        save_users(data)
    return data
...
def create_default_files(server_dir):
    ...
    if not os.path.exists(main_py):
        with open(main_py, 'w', encoding='utf-8') as f:
            f.write('''# MAHIR HOSTING - Default Bot
import time

print("=" * 40)
print("Bot is running on MAHIR HOSTING")
...
def api_create_server():
    ...
    if not username:
        username = f"THEMAHIRWORLD{random.randint(10000, 99999)}"
...
def login():
    if request.method == 'POST':
        ...
        # অ্যাডমিন চেক (যেকোনো অ্যাডমিন ইউজার)
        ...
        # ইউজার চেক (সার্ভার এক্সেসের জন্য)
        if username in users and users[username].get('role') == 'user':
            if users[username].get('password') == password:
...
if __name__ == '__main__':
    print("\n" + "=" * 50)
    print("🚀 MAHIR HOSTING - FINAL")
    print("=" * 50)
    print("📍 Landing: http://localhost:8080")
    print("📍 Admin: http://localhost:8080/login")
    print("🔗 API: http://localhost:8080/api/create")
    print("👤 MAHIR / MAHIRJOD")
    print("=" * 50 + "\n")
